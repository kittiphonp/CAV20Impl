...
public class STPGModelChecker extends ProbModelChecker
{
			...
			throws PrismException
	{
		...
		timer = System.currentTimeMillis();
		if (verbosity >= 1)
			mainLog.println("Starting SG !BOUNDED! value iteration (" + (min1 ? "min" : "max") + (min2 ? "min" : "max") + ")... Probably only working for maxmin");
...
//			}
		...
		mainLog.println("Computing MSEC done in " + (double) duration0 / 1000 + " secs.");
		*/
...
//			//=============== UNTIL HERE =================
			...
			//	//mainLog.println(mec);
				...
				//mainLog.println("Computing MSEC done in " + (double) duration0 / 1000 + " secs.");
				for (int j = 0; j<simpleBCECs.size();j++){
				//for (int j = simpleBCECs.size()-1; j >=0; j--){
					BitSet simBCEC = simpleBCECs.get(j);
