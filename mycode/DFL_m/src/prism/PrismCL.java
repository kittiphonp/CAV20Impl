	...
	private void doParsing()
	{
		...
		try {
			if (importpepa) {
				...
				mainLog.print("\nParsing the model file \"" + modelFilename + "\"...\n");
	...
	private void parseArguments(String[] args) throws PrismException
	{
		...
		for (i = 0; i < args.length; i++) {
			...
			if (args[i].length() > 0 && args[i].charAt(0) == '-') {
				...
				// use heuristics for solving
				else if (sw.equals("heuristic")) {
					if (i < args.length - 1) {
						prism.setHeuristic(true);
						prism.setHeuristicName((String)args[++i]);

					} else {
						errorAndExit("No heuristic name specified");
					}
				}
				// next state strategy
				else if (sw.equals("next_state")) {
					if (i < args.length - 1) {
						prism.setHeuristicNextStateName((String)args[++i]);
					} else {
						errorAndExit("No heuristic name specified");
					}
				}
				// next state strategy
				else if (sw.equals("heuristic_verbose")) {
					prism.setHeuristicVerbose(true);
				}
				// next state strategy
				else if (sw.equals("heuristic_epsilon")) {
					if (i < args.length - 1) {
						prism.setHeuristicEpsilon(Double.parseDouble((String)args[++i]));
					} else {
						errorAndExit("No epsilon specified");
					}
				}
				//RTDP_ADJ optimization switch
				else if (sw.equals("RTDP_ADJ_OPTS")) {
					if (i < args.length - 1) {
						prism.setRTDP_ADJ_optimizations(Integer.parseInt((String)args[++i]));
					} else {
						errorAndExit("No RTDP_ADJ_OPTS specified");
					}
				}
