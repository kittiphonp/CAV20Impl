...
public class STPGModelChecker extends ProbModelChecker
{
			...
			throws PrismException
	{
		...
		// but we do not care about it here because we just want model.txt
		/*
	...
	// NEW ALGORITHM IS HERE. - learning version
			...
			throws PrismException
	{
		...
		BitSet unknown;
		int i, n, iters;
		double lowerBounds[], upperBounds[], initVal;
		int adv[] = null;
		boolean genAdv, done;
		long timer;
		...
		timer = System.currentTimeMillis();
		if (verbosity >= 1)
			mainLog.println("Starting BOTTLENECK+LEARNING technique (" + (min1 ? "min" : "max") + (min2 ? "min" : "max") + ")... Probably only working for maxmin");
		...
		// where initVal is 0.0 or 1.0, depending on whether we converge from below/above.
		for (i = 0; i < n; i++) {
			lowerBounds[i] = yes.get(i) ? 1.0 : no.get(i) ? 0.0 : 0.0;
			upperBounds[i] = yes.get(i) ? 1.0 : no.get(i) ? 0.0 : 1.0;
		}
		...
		try {
			...
			for (int ii = 0; ii < n; ii++) {
				out.println(stpg.getPlayer(ii)-1);
						/*
						if (yes.get(ii)) {
							out.println("1.0 " + n);
						}
						 ...
						 */
				for (int a = 0; a < stpg.getNumChoices(ii); a++) {
					Distribution distr = ((SMG) stpg).trans.get(ii).get(a);
					for (Map.Entry<Integer, Double> e : distr) {
						int t = e.getKey();
						double prob = e.getValue();
						out.print(prob + " " + t + " ");
					}
					if (a != stpg.getNumChoices(ii)-1)
						out.print(", ");
				}
				out.println();
				// }
		...
		mainLog.println("#state visits = "+visited.size());
		mainLog.println("#state total  = "+n);
		mainLog.println();
