...
public class HeuristicsSMGModelChecker extends PrismComponent
{
	...
	public HeuristicsSMGModelChecker(PrismComponent parent, ModulesFile modulesFile, PropertiesFile propertiesFile, SimulatorEngine engine, String heuristicName, String nextStateName, boolean verbose, double epsilon, int RTDP_ADJ_optimizations) throws PrismException
	{
		super(parent);
		this.modulesFile = modulesFile;
		this.propertiesFile = propertiesFile;
	...
	private Result checkExpressionProb(ExpressionProb expr) throws PrismException
	{
		...
		Heuristic h = HeuristicFactory.createHeuristic(type,this, stateUpdate, nextState, modelExplorer, min);
		h.setBound(bound);
		...
		//added this: if we are a HeuristicRTPD_Adj, we need to set the SMG coalition
		if(type==HeuristicType.RTDP_ADJ){
			((HeuristicRTDP_Adj) h).setCoalition(playersInCoalition,modulesFile.getNumPlayers());
			((HeuristicRTDP_Adj) h).setOpts(RTDP_ADJ_optimizations);
			if (RTDP_ADJ_optimizations>127){
				((HeuristicRTDP_Adj) h).setIsMDP(true);
			}
		}
