...
//            } else {
                bestAction = stateUpdate.update(currentState);
...
//            }
            ...
            // create new graph
            LinkedList<STPGModelChecker.Triplet<Double, Integer, Integer>> G[] = new LinkedList[n];
            ...
            for (int s = 0; s < n; s++) {
                ...
                if (smg.getPlayer(s) != 1) {
                    pme.queryState(s);
                    int num_actions = pme.getNumChoices();
                    for (int a = 0; a < num_actions; a++) {
                        pme.queryState(s);
                        int num_succ = pme.getNumTransitions(a);
                        for (int t = 0; t < num_succ; t++) {
                            pme.queryState(s);
                            double prob = pme.getTransitionProbability(a,t);
                        ...
                        for (Map.Entry<Integer, Double> e : distr) {
                            int t = e.getKey();
                            G[t].add(new STPGModelChecker.Triplet(-up_val, s, t));
                        }
                    }
                }
                // Minimizer
            ...
            Comparator<STPGModelChecker.Triplet<Double, Integer, Integer>> cmp = (t1, t2) -> t1.get1() < t2.get1() ? -1 : 1;

            PriorityQueue<STPGModelChecker.Triplet<Double, Integer, Integer>> heap = new PriorityQueue<>(cmp);
            ...
            while (!heap.isEmpty()) {
                STPGModelChecker.Triplet<Double, Integer, Integer> top = heap.remove();
                double v = top.get1();
                int s = top.get2();
                int t = top.get3();
                if (bound[s] <= 1) {
                ...
                bound[s] = -v < bound[t] ? -v : bound[t];
                heap.addAll(G[s]);
            }

            for (int s = 0; s < n; s++) {
                if (bound[s] > 1) {
                    stateUpdate.setQValue(smg.getStatesList().get(s), new StateValue(lowerBounds[s], 0));
                }
                if (bound[s] < upperBounds[s]) {
                    stateUpdate.setQValue(smg.getStatesList().get(s), new StateValue(lowerBounds[s], bound[s]));
                }
            }
            */
