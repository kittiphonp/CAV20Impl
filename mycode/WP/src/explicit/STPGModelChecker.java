...
public class STPGModelChecker extends ProbModelChecker
{
			...
			throws PrismException
	{
		...
		timer = System.currentTimeMillis();
		if (verbosity >= 1)
			mainLog.println("Starting SG WPP technique (" + (min1 ? "min" : "max") + (min2 ? "min" : "max") + ")... Probably only working for maxmin");
		...
		while (!done ){ //&& iters < maxIters) { random breaking no longer
			...
			*/
			...
			// mainLog.println(iters + " " + (upperBounds[initialState] - lowerBounds[initialState]));
			done = upperBounds[initialState] - lowerBounds[initialState] < eps;
...
//			}
		...
		// timing EC computation
        /*
		BitSet all = new BitSet();
		all.set(0,stpg.getNumStates());
		all.xor(finished);
		explicit.ECComputerDefault ec = (ECComputerDefault) ECComputer.createECComputer(this, stpg);
		long start_ec_time = System.currentTimeMillis();
		ec.computeMECStates(all);
		long duration_ec_time = System.currentTimeMillis() - start_ec_time;
		mainLog.println("finding EC in " + (double) duration_ec_time / 1000 + " secs.");
        */
